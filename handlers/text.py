...
• `rag` - **Технический каталог** (основной режим для работы с продукцией)
• `text` - Общий диалог с GPT-4o
• `voice` - Голосовой режим (с синтезом речи)
• `vision` - Анализ изображений, чертежей, фото

**Рекомендация:**
Для вопросов о люках и дождеприемниках используйте режим `rag`

**Использование:**
/mode <название_режима>

**Примеры:**
/mode rag - переключиться на работу с каталогом
/mode vision - проанализировать чертеж"""
    ...
    mode_descriptions = {
        ...
        BotMode.VISION: "📸 Режим Vision - анализ изображений и чертежей",
        BotMode.RAG: "📚 Режим работы с техническим каталогом - точные ответы по продукции"
    }
